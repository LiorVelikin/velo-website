        ...
        <div className="p-6">
          <div className="flex flex-wrap gap-2 mb-4">
            {project.tags.map((tag) => (
              <span key={tag} className="text-[10px] font-semibold text-[#a0b4cc] bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] px-2 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
          <h3 className="text-lg font-bold text-white mb-1">{project.title}</h3>
          <p className="text-[#8ba3c7] text-sm font-medium mb-3">{project.subtitle}</p>
          <p className="text-[#6a849e] text-sm leading-relaxed">{project.description}</p>

          <div className="mt-5 flex items-center gap-2 text-sm font-semibold group-hover:gap-3 transition-all duration-200" style={{ color: project.accent }}>
